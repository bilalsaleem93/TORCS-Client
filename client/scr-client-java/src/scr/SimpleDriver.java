...
public class SimpleDriver extends Controller{
	...
	final int[]  gearUp={7500,7500,8000,8000,8200,0};
	final int[]  gearDown={0,2500,4000,4300,4500,45000};
	...
	private float getSteer(SensorModel sensors){
			 ...
			 if(speed>23)
			  steerLock=(float) 0.007000;
	     	 else 
			   steerLock=(float) 0.845500;
	...
	{ 
	    ...
	    {  /*
			...
			//float r_t=(float) sensors.getTrackEdgeSensors()[10];
	        float rxSensor=(float) sensors.getTrackEdgeSensors()[10];//+r_t)/2.90;
	        // reading of sensor parallel to car axis
			//float l_t(float) sensors.getTrackEdgeSensors()[7];
	        float sensorsensor=(float) sensors.getTrackEdgeSensors()[9];
	        // reading of sensor at -5 degree w.r.t. car axis
	        float sxSensor=(float) sensors.getTrackEdgeSensors()[8];//+l_y)/2.90;

	        float targetSpeed;

	        // track is straight and enough far from a turn so goes to max speed
	        if (sensorsensor>maxSpeedDist || (sensorsensor>=rxSensor && sensorsensor >= sxSensor))
			    {//steerLock=(float) 0.045500;
	        ...
	        else
	        {
	            ...
	            {  //steerLock=(float) 0.005500;
	                // computing approximately the "angle" of turn
	                float h = sensorsensor*sin5;
	                float b = rxSensor - sensorsensor*cos5;
	                float sinAngle = b*b/(h*h+b*b);
					//system.out.println("right"+rxSensor);
	                // estimate the target speed depending on turn and on how close it is
	                targetSpeed = maxSpeed*(sensorsensor*sinAngle/maxSpeedDist)+10;
	            ...
	            {  //steerLock=(float) 0.005500;
	                ...
	                // estimate the target speed depending on turn and on how close it is
	                targetSpeed = maxSpeed*(sensorsensor*sinAngle/maxSpeedDist)+10;
					//system.out.println("right"+sxSensor);
	            }
...
/*float slip;			 
			 for (int i = 0; i < 4; i++)
	    {
	       slip += sensors.getWheelSpinVelocity()[i] * wheelRadius[i];
			...
			float acc_brake= (2/(1+Math.exp(sensors.getSpeed() - targetSpeed))+0.1);
			acc_brake = acc_brake - (float)((sensors.getSpeed() / 3.6) - slip -1.5)/5 ;*/
	        
			float speed = (float) (sensors.getSpeed() / 3.6);
		float rxSensor=(float) sensors.getTrackEdgeSensors()[10];
		float sxSensor=(float) sensors.getTrackEdgeSensors()[8];
		float fSensor=(float) sensors.getTrackEdgeSensors()[9];
		float estimate_speed = (float)alpha*(rxSensor+sxSensor+fSensor)+beta;
		float difference = (float)estimate_speed - speed;
			
			return (float) ((difference/estimate_speed)*gamma)+(float)0.4;
	...
	private float filterABS(SensorModel sensors,float brake){
		...
		float sxSensor=(float) sensors.getTrackEdgeSensors()[8];
		float fSensor=(float) sensors.getTrackEdgeSensors()[9];
		estimate_speed = 0.5*(rxSensor+sxSensor+fSensor)+0.5;
		difference = estimate_speed - speed;
		brake=(difference/estimate_speed)*.5;
		*/
